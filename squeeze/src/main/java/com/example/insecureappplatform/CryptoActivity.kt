    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnEncrypt.setOnClickListener {
            ...
            if (plainText.isBlank()) {
                tvResult.text = "ギフトメッセージを入力してください。"
            ...
            val raw = Base64.encodeToString(encrypted, Base64.NO_WRAP)
            val code = raw.filter { it.isLetterOrDigit() }.take(12).uppercase().chunked(4).joinToString("-")
            getSharedPreferences("gift_cards", Context.MODE_PRIVATE).edit()
                .putString("last_gift_blob", raw)
                .putString("last_gift_code", "SQZ-$code")
                .apply()
            tvResult.text = "ギフトコードを発行しました\n\nSQZ-$code\n\nこのコードを受け取る方へ共有してください。"
        }
    }
}
