...
class NetworkActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val store = getSharedPreferences("squeeze_store", Context.MODE_PRIVATE)
        val items = store.getString("cart_items", "").orEmpty().split("|").filter { it.isNotBlank() }
        findViewById<TextView>(R.id.tv_order_items).text = items.joinToString("\n") { "・$it" }
        findViewById<TextView>(R.id.tv_order_total).text = "合計  ¥${store.getInt("cart_total", 0)}"

        tvDelivery = findViewById(R.id.tv_delivery)
        btnCheckout = findViewById(R.id.btn_checkout)
        refreshCustomer()

        // 通常UIでは通信方式を選ばせない。教材用の lab_config だけが経路を切り替える。
        btnCheckout.setOnClickListener {
            val customer = getSharedPreferences("user_session", Context.MODE_PRIVATE)
                .getString("username", null)
            if (customer == null) {
                startActivity(Intent(this, InsecureStorageActivity::class.java))
                return@setOnClickListener
            }
            when (getSharedPreferences("lab_config", Context.MODE_PRIVATE)
                .getString("network_mode", "pinned")) {
                "http" -> send("http://10.0.2.2:8080/login", pinned = false)
                "https" -> send("https://10.0.2.2:8443/login", pinned = false)
                else -> send("https://10.0.2.2:8443/login", pinned = true)
            }
        }
    }
    ...
    override fun onResume() {
        super.onResume()
        if (::btnCheckout.isInitialized) refreshCustomer()
    }

    private fun refreshCustomer() {
        val customer = getSharedPreferences("user_session", Context.MODE_PRIVATE)
            .getString("username", null)
        tvDelivery.text = buildString {
            appendLine(customer ?: "会員情報が登録されていません")
            append("通常配送（2〜3営業日）")
        }
        btnCheckout.text = if (customer == null) "会員情報を登録する" else "注文を確定する"
    }
    ...
    private fun send(endpoint: String, pinned: Boolean) {
        tvResult.text = "注文を送信しています…"
        val store = getSharedPreferences("squeeze_store", Context.MODE_PRIVATE)
        val body = credentials() +
            "&items=${store.getInt("cart_count", 0)}" +
            "&total=${store.getInt("cart_total", 0)}"
        Thread {
            var success = false
            val detail = try {
            ...
            Log.d("SqueezeNetwork", detail)
            runOnUiThread {
                if (success) {
                    completeOrder(store)
                    tvResult.text = "ご注文を受け付けました\n注文履歴から配送状況を確認できます。"
                } else {
                    tvResult.text = "注文を送信できませんでした\n接続を確認して、もう一度お試しください。"
                }
            }
        }.start()
    }

    private fun completeOrder(store: android.content.SharedPreferences) {
        val total = store.getInt("cart_total", 0)
        store.edit()
            .putString("last_order_id", "SQ-${UUID.randomUUID().toString().take(8).uppercase()}")
